...
export default function AnalyticsDashboard({ propertyId }: { propertyId: string }) {
      ...
      const mealMetrics: MetricCard[] = [
        {
          ...
          icon: <ShoppingCart className="h-5 w-5 text-aubergine" />
        },
        {
  ...
  if (loading) {
    return <div className="text-center py-12 text-ink/50">Loading analytics...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 mb-6">
        <BarChart3 className="h-6 w-6 text-aubergine" />
        <h2 className="text-2xl font-display text-aubergine">Operations Analytics</h2>
      </div>
        ...
        {metrics.map((metric, idx) => (
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-ink/60 mb-2">{metric.label}</p>
              ...
              <div className="text-aubergine opacity-50">{metric.icon}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {spending.length > 0 && (
        <div className="bg-white rounded-2xl p-6 border border-gold-light/20 shadow-sm">
          <h3 className="font-semibold text-ink mb-4">Projected Spending by Category</h3>
              ...
              return (
                <div key={item.category}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <p className="text-sm font-bold text-aubergine">${item.amount.toFixed(2)}</p>
                  </div>
                  <div className="w-full bg-gold-light/20 h-2 rounded-full overflow-hidden">
                    <div
                      className="bg-aubergine h-full transition-all duration-300"
          ...
          <div className="mt-4 pt-4 border-t border-gold-light/20">
            <p className="text-sm text-ink/60">
              Total Projected Cost: <span className="font-bold text-aubergine">${totalSpending.toFixed(2)}</span>
            </p>
          </div>
        </div>
      )}
