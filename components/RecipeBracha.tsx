...
// hardcoded label map, so this never drifts from bracha_categories.
...
export default function RecipeBracha({
  ...
  function handleSave() {
    startTransition(async () => {
      ...
      if (result.success) {
        setSaved(selected);
        showToast('Bracha saved.', { variant: 'success' });
      } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm shadow-charcoal/5 p-4 print:hidden">
      <h3 className="font-display text-lg text-charcoal mb-1">Bracha</h3>
