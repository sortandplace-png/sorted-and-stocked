...
export default function HouseholdKnowledgeClient({ propertyId }: { propertyId: string }) {
  ...
  // Manager-only per spec (SS-036) -- was only gating the add/delete
  // controls, so any staff member could browse the full list. Same
  // view-level block CaptureInboxClient.tsx already uses for its own
  ...
  return (
    <div className="max-w-md mx-auto p-4">
      ...
      {existingDraft && (
        <div className="bg-mist border border-brass/30 rounded-2xl p-3 mb-4 text-sm">
          <p className="text-denim mb-2">You have an unsaved draft entry.</p>
      ...
      <div className="bg-card rounded-2xl border border-cardBorder shadow-card p-4 mb-6 space-y-2">
        <h2 className="font-display text-lg text-denim mb-1">Add an entry</h2>
        <div>
      ...
      {Object.keys(grouped).length === 0 && (
        <p className="text-sm text-dusk text-center py-8">
          {entries.length === 0 ? 'No entries yet — use the form above to add your first Q&A.' : 'No matches.'}
        </p>
      )}
      ...
      <div className="space-y-6">
        {Object.entries(grouped).map(([cat, catEntries]) => (
          <div key={cat}>
            <h3 className="text-xs font-medium uppercase tracking-wider text-brass mb-2">{cat}</h3>
            <ul className="space-y-2">
              {catEntries.map((entry) => (
                <li key={entry.id} className="bg-card rounded-xl border border-cardBorder shadow-card p-3">
                  <div className="flex items-start justify-between gap-2">
                    <p className="font-medium text-sm text-denim">{entry.question}</p>
                    <button
                  ...
                  <p className="text-sm text-dusk mt-1">{entry.answer}</p>
                </li>
              ))}
            </ul>
          </div>
