  ...
  return (
    <div className="max-w-md lg:max-w-2xl mx-auto p-4">
      <Link
        ...
        className="text-sm text-aubergine font-medium mb-3 inline-block"
      >
        ← Meal plan
      </Link>
      <h1 className="text-2xl font-display text-aubergine mb-1">Yom Tov — the year ahead</h1>
      ...
      {loading && <p className="text-sm text-ink/40 text-center mt-8">Loading calendar…</p>}
      {error && <p className="text-sm text-rust bg-rust/10 rounded-xl px-3 py-2">{error}</p>}

      {!loading && !error && entries.length === 0 && (
        <p className="text-sm text-ink/40 text-center mt-8">No Yom Tov or fast dates found.</p>
      )}

      <div className="space-y-5">
        {[...grouped.entries()].map(([monthKey, monthEntries]) => (
          <div key={monthKey}>
            <h2 className="text-xs font-display italic tracking-[0.1em] text-aubergine/70 mb-2">
              {new Date(monthKey + '-02').toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}
            </h2>
            <ul className="divide-y divide-gold-light/30 rounded-2xl bg-white shadow-sm shadow-aubergine/5 overflow-hidden">
              {monthEntries.map((entry, i) => (
                <li key={i} className="flex items-center gap-3 px-4 py-3">
                  <span
                    className={
                      'text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 ' +
                      (entry.isFast ? 'text-rust bg-rust/10' : 'text-cream bg-aubergine')
                    }
                  >
                    {entry.isFast ? '🕯️ Fast' : '✡︎ Yom Tov'}
                  </span>
                  <span className="flex-1 text-sm text-ink truncate">{entry.title}</span>
