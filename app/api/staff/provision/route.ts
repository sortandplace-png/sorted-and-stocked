...
async function sendProvisionEmail(opts: {
  ...
  const roleLabelEn = opts.role === 'manager' ? 'manager' : opts.role === 'owner' ? 'owner' : 'staff';
  const roleLabelEs = opts.role === 'manager' ? 'gerente' : opts.role === 'owner' ? 'propietario' : 'personal';
