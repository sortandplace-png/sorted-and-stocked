...
// SS-631: pin dot required on every card, no exceptions -- including the
// large denim card, which is FLAGGED-not-changed as a background but
// still needs its dot.
...
// SS-631: brass is NEVER a fill -- hairlines, eyebrow labels, icon
// strokes and the pin dot only. This mark was three solid brass shapes.
// Now an outline: same geometry, stroked rather than filled, which is
// what the rule actually asks for and reads sharper at 34px anyway.
const HOUSE_MARK = (
  <svg viewBox="0 0 100 100" aria-hidden="true" className="w-full h-full">
    <g fill="none" stroke="#C6A46E" strokeWidth="6" strokeLinejoin="round" strokeLinecap="round">
      <rect x="64" y="14" width="9" height="27" />
      <polygon points="50,8 92,44 8,44" />
      <rect x="20" y="44" width="60" height="45" />
    </g>
  </svg>
);
    ...
    <div className="bg-linen py-8 px-4">
      <div className="max-w-[1080px] mx-auto">
        ...
        <section className="grid md:grid-cols-2 gap-10 items-center py-12">
          <div>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              {/* SS-631: Request Early Access appeared TWICE in one
                  viewport -- here and in the nav above. The nav one stays
                  (it remains reachable further down the page); this one
                  goes, leaving the hero with a single secondary link. */}
          ...
          <div className="relative bg-denim rounded-xl3 p-6 shadow-card">
            <Pin />
        ...
        <section className="py-14">
          <p className="text-[11px] tracking-[0.18em] uppercase font-bold text-brass mb-3.5">The Real Problem</p>
          ...
          <div className="grid md:grid-cols-2 gap-3.5">
            {[
              ...
              <div key={title} className="relative bg-card border border-cardBorder rounded-xl2 shadow-card p-5">
                <Pin size="sm" />
      ...
      <div className="max-w-[1080px] mx-auto">
        <section className="py-14">
          <p className="text-[11px] tracking-[0.18em] uppercase font-bold text-brass mb-3.5">What&apos;s Inside</p>
          <h2 className="font-display font-semibold text-3xl text-denim mb-6">Everything a household actually runs on.</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {FEATURES.map((f) => (
              <div key={f.title} className="relative bg-card border border-cardBorder rounded-xl2 shadow-card p-5">
                <Pin size="sm" />
        ...
        <section className="grid md:grid-cols-[0.4fr_0.6fr] gap-11 items-center py-14 text-center md:text-left">
          ...
          <div>
            <p className="text-[11px] tracking-[0.18em] uppercase font-bold text-brass mb-3.5">Built By Someone Who&apos;s Done This</p>
